...
- [x] 2.1 将 Chaos Mesh Helm 仓库添加到部署脚本
  - 编辑 `Makefile` 或部署脚本，添加 Chaos Mesh repo
  - 使用官方 Helm repo: `https://charts.chaos-mesh.org`
  - 验证 repo 可访问

- [x] 2.2 创建 `deploy/monitoring/chaos-mesh-values.yaml` 并配置设置
...
- [x] 2.3 为 Chaos Mesh 镜像配置 `imagePullSecrets`（如需要）
  - 在 values.yaml 中配置 `imagePullSecrets`
  - 验证镜像可拉取
  - 如使用私有镜像，提前准备好 secret

- [x] 2.4 使用 `helm upgrade --install` 将 Chaos Mesh 部署到 monitoring 命名空间
  - 执行 helm install 命令
  - 验证所有 Pod 正常启动
  - 验证 CRD 已安装（`kubectl get crd | grep chaos`）

- [x] 2.5 验证 Chaos Mesh 控制器和 webhook Pod 健康状态
...
- [x] 3.4 配置准入 Webhook 进行实验验证
  - 启用 validation webhook
  - 配置验证规则（命名空间、资源、参数范围）
  - 测试验证逻辑

- [x] 3.5 验证 Chaos Mesh CRD 已安装且功能正常
...
- [x] 4.1 创建 pod-failure 实验模板（deploy/chaos/pod-failure.yaml）
  - 配置 `PodChaos` 资源
  - 设置 `action: pod-kill`
...
- [x] 4.2 创建 network-latency 实验模板（deploy/chaos/network-latency.yaml）
  - 配置 `NetworkChaos` 资源
  - 设置 `action: delay`
...
- [x] 4.3 创建 dependency-failure 实验模板（deploy/chaos/dependency-failure.yaml）
...
- [x] 4.4 创建 resource-exhaustion 实验模板（deploy/chaos/resource-exhaustion.yaml）
  - 配置 `StressChaos` 资源
...
- [x] 4.5 创建 cascade-failure 实验模板（deploy/chaos/cascade-failure.yaml）
  - 创建多个 Chaos 资源在单个文件中
...
- [x] 5.1 配置 Prometheus 抓取 Chaos Mesh 控制器指标
...
- [x] 7.1 创建混沌实验运行手册（docs/chaos/runbook.md）
  - 文档结构：
    - 实验概述
    - 前置条件（告警覆盖度检查）
    - 执行步骤
...
- [x] 7.3 创建告警覆盖度检查指南（docs/chaos/alert-coverage-check-guide.md）
  - 告警覆盖度检查的目的和重要性
  - 关键告警场景清单
  - 告警覆盖度检查流程
  - 告警覆盖率阈值定义
...
- [x] 7.4 创建告警验证指南（docs/chaos/alert-validation-guide.md）
  - 告警验证的目的和重要性
  - 告警验证流程
  - 如何识别告警缺失
  - 如何识别告警误报
...
- [x] 7.5 创建自我恢复能力验证指南（docs/chaos/self-healing-validation-guide.md）
  - 自我恢复能力的定义和重要性
...
- [x] 7.7 记录 Chaos Mesh CLI 和 Web UI 使用方法
  - CLI 命令：
...
- [x] 8.1 创建 `/chaos-check-alerts` skill
...
- [x] 8.2 创建 `/chaos-inject-pod` skill
...
- [x] 8.3 创建 `/chaos-inject-network` skill
...
- [x] 8.4 创建 `/chaos-inject-dependency` skill
...
- [x] 8.5 创建 `/chaos-inject-resource` skill
...
- [x] 8.6 创建 `/chaos-inject-cascade` skill
...
- [x] 8.8 创建 `/chaos-validate-alerts` skill
...
- [x] 8.9 创建 `/chaos-validate-self-heal` skill
...
- [x] 8.12 Skills 集成测试
  ...
  - **验收标准**:
    - [x] 所有 skills 都能正常工作
    - [x] 所有故障类型都可以通过 skills 触发
    - [x] 所有验证流程都能通过 skills 完成
    - [x] 实验报告能正确生成
    - [x] Runbook 能正确更新

## 9. Skills 功能验证测试

- [x] 9.1 验证 `/chaos-check-alerts` skill
...
- [x] 9.2 验证 `/chaos-inject-pod` skill
...
- [x] 9.3 验证 `/chaos-inject-network` skill
...
- [x] 9.4 验证 `/chaos-inject-dependency` skill
...
- [x] 9.5 验证 `/chaos-inject-resource` skill
...
- [x] 9.6 验证 `/chaos-inject-cascade` skill
...
- [x] 9.8 验证 `/chaos-validate-alerts` skill
...
- [x] 9.9 验证 `/chaos-validate-self-heal` skill
...
- [x] 9.1 在开发/测试环境测试 pod-failure 实验
  - **前置检查**: 运行告警覆盖度检查，确保告警覆盖充分
  - **执行**: 应用 pod-failure 实验
  - **观察**: Pod 重建时间、Service endpoints 更新、错误率 spike、告警触发
  - **验证**: 参考 Pod 故障注入全流程验收标准
  - **自我恢复能力验证**:
    - [x] Pod 自动重建并在 60s 内 Ready
    - [x] Service 自动更新 endpoints
    - [x] 流量自动切换到健康 Pod
    - [x] 无资源泄漏（连接、文件描述符）
    - [x] 无需人工干预完全恢复
  - **告警验证**:
    - [x] PodNotReady 告警在 30s 内触发
    - [x] 告警标签正确
    - [x] PodReady 后告警在 60s 内恢复
    - [x] 无预期外的告警误报
  - **验收标准**:
    - [x] Pod 在 60s 内恢复 Ready
    - [x] Service endpoints 在 30s 内更新
    - [x] 错误率 spike 不超过 10%
    - [x] 5 分钟内 QPS 恢复到基准的 90% 以上
    - [x] 无持久化资源泄漏
    - [x] **自我恢复验收标准**: 全部通过
    - [x] **告警验收标准**: 全部通过

- [x] 8.2 在开发/测试环境测试 network-latency 实验
  - **前置检查**: 运行告警覆盖度检查，确保告警覆盖充分
  ...
  - **自我恢复能力验证**:
    ...
    - [x] 重试机制自动恢复
  - **告警验证**:
    - [x] HighLatency 告警在 P95 超过阈值后触发
    - [x] High5xxRate 告警在错误率超过阈值后触发
    - [x] 延迟恢复后告警及时恢复
    - [x] 告警标签准确
  - **验收标准**:
    - [x] P95 延迟 spike 符合注入量 + 处理时间
    - [x] 超时仅在延迟超过配置阈值时发生
    - [x] 重试次数符合配置策略
    - [x] 移除延迟后无"慢恢复"现象
    - [x] 无级联故障
...
- [x] 8.3 在开发/测试环境测试 dependency-failure 实验
  - **前置检查**: 运行告警覆盖度检查，确保告警覆盖充分
  ...
  - **自我恢复能力验证**:
    ...
    - [x] 缓存自动预热
  - **告警验证**:
    - [x] ServiceUnavailable 告警及时触发
    - [x] DependencyErrorRateHigh 告警准确触发
    - [x] CircuitBreakerOpen 告警在熔断器打开时触发
    - [x] 服务恢复后告警及时恢复
  - **验收标准**:
    - [x] 依赖故障不导致级联故障
    - [x] 熔断器正确打开和恢复
...
- [x] 8.9 验证防护机制（命名空间限制、资源限制）
  - **执行**: 尝试在禁止命名空间创建实验
  - **执行**: 尝试创建超限参数的实验
  - **观察**: 是否被拒绝
  - **验收标准**:
    - [x] 非 online-boutique 命名空间实验被拒绝
    - [x] monitoring/kube-system 实验被拒绝
    - [x] 超限参数被拒绝
    - [x] 错误消息清晰明确